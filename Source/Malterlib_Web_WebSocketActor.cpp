...
namespace NMib::NWeb
{
	...
	namespace
	{
		...
		struct CMessage
		{
			...
			umint m_Position = 0;
			// Bytes that went through the incoming pages while the message was being assembled
			// from receive buffer views: the frame headers and control frames between its
			// fragments. They pin the buffers they share with the payload without being part of
			// it, so past a budget the rest of the payload is copied instead of referenced
			umint m_nInterleavedBytes = 0;
			uint8 m_Mask[4];
			CHeader m_Header;
		...
		// The most bytes of control frames and frame headers a message assembled from receive
		// buffer views may have between its fragments before the rest of it is copied: a peer
		// padding the gaps with control frames would otherwise make the message pin a buffer
		// per fragment, capacity the receive window is not sized for
		constexpr static umint gc_MaxInterleavedBytes = 64 * 1024;
	...
	// bounds it and the window is bounded — so a send never grows it
		...
		-> umint
	{
		...
		// the kernel is done with the pages by the time the transfer is reported. Neither has a
		// readiness send, which hands the bytes to the kernel before it returns
	...
	NConcurrency::TCFuture<void> CWebSocketActor::fp_Destroy()
	{
		...
		// A destroy tears the connection down at once, close reply or shutdown still on their
		// way out included: the close resets, so the peer sees the connection fail rather than
		// close, and the kernel lets go of the zero copy pages instead of retransmitting them,
		// which with an unresponsive peer would keep the fence below waiting for the
		// retransmission lifetime. That is the contract; a connection that should close cleanly
		// goes through f_CloseWithLinger, which drains and shuts down before it destroys, and
		// whose timeout is what brings an unresponsive peer here. Do not make the destroy wait
		// for any of that
	...
	NConcurrency::TCFuture<CWebSocketActor::CCloseInfo> CWebSocketActor::f_CloseWithLinger(EWebSocketStatus _Status, NStr::CStr _Reason, fp64 _MaxLingerTime)
	{
...

		{
			...
			// the close reply the peer is owed, parked behind the messages queued ahead of it,
			// or waiting for the peer's hangup after it. Destroying it here would close the
			// socket under those sends, which a completion loop cancels, and the peer would see
			// the connection fail instead of close; it lingers below like any other, and the
			// shutdown callback delivers the close info once the drain is done
			if (!Internal.m_pSocket)
			{
	...
	void CWebSocketActor::fp_StartReceiveStream()
	{
		...
		// A message assembled from zero copy views keeps every buffer its frames landed in until
		// it is delivered whole: up to a maximum sized message plus the control frames and
		// headers between its fragments, up to the interleave budget. That hold is stated to
		// the stream, which knows how large its buffers are and how many fill at once, and it
		// raises the window and the resume threshold to cover the partly used buffers the hold
		// pins on top: parked with the final frame still to come, the releases of everything
		// else must bring the count under the threshold, or nothing would ever resume. An
		// override stays what the test asked for
	...
	void CWebSocketActor::fp_ReceiveStreamInput(NSys::CIoStreamSegment &&_Segment, bool _bHeldOnly)
	{
					...
					while (iOffset < SharedData.f_GetLen())
					{
						...
						if (Internal.m_nDirectReadRemaining)
						{
							...
							if (Internal.m_bDirectReadToStorage)
							{
								...
								// the consumer lets go of the message. A message whose fragments
								// are separated by more than the interleave budget copies instead,
								// so it pins no more buffers than its size accounts for
						...
						else
						{
							...
							// Between the fragments of a message assembled from views, these bytes
							// are what keeps the buffers pinned without being payload; a fragment's
							// own prefix is taken back off the count when its direct read starts
		...
		try
		{
			for (;;)
			{
				// A terminal carries no bytes and resolves without a destination. That is also
				// the one way a shared connection gets here — its data resolves in the loop
				// above — so a storage direct read the peer's close cut short may still be
				// pending, with no buffer to name
	...
	// deferral itself no longer applies: the handshake is over. The re-entry is deferred to the
	// state processing already running, if the disconnect that ended the handshake came from one
 ...
 */
	...
	void CWebSocketActor::fp_ProcessState(NNetwork::ENetTCPState _StateAdded)
	{
		...
		)
		{
			...
			// rule below — and the stream always terminates once the peer is gone. A connection
			// whose handshake is over waits for nothing: data after a close frame is a protocol
			// violation, and an application holding delivered buffers parks every receive, so the
			// terminal that would release the close might never be reported at all
