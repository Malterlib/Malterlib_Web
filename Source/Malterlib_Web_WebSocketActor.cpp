...
namespace NMib::NWeb
{
	...
	void CWebSocketActor::fp_StartReceiveStream()
	{
		...
		// A message assembled from zero copy views pins every buffer its frames landed in until
		// it is delivered whole — up to a maximum sized message and the partly used buffers at
		// either end of it — so the window holds that and one buffer more, and the resume
		// threshold sits no lower than that: parked with the final frame still to come, the
		// releases of everything else must bring the count under the threshold, or nothing would
		// ever resume. An override stays what the test asked for
		pBackpressure->m_nResumeBytes = pBackpressure->m_nLimitBytes / 2;
		if (!Internal.m_pIo->f_ReceiveWindowBytesOverride())
		{
			umint nMessagePinnable = fg_Min(Internal.m_Settings.m_MaxMessageSize, TCLimitsInt<umint>::mc_Max - 3 * nBufferBytes) + 2 * nBufferBytes;
			pBackpressure->m_nLimitBytes = fg_Max(pBackpressure->m_nLimitBytes, nMessagePinnable + nBufferBytes);
			pBackpressure->m_nResumeBytes = fg_Max(pBackpressure->m_nLimitBytes / 2, nMessagePinnable);
		}
	...
	void CWebSocketActor::fp_SubmitSendOp(bool _bContinue, umint _iInheritedReservation)
	{
		...
		NNetwork::FSocketSendReleased fOnReleased = [Hold = NConcurrency::CIoCompletionOpHold(Internal.m_pOpTracker), KeepAlives = fg_Move(KeepAlives), pArenaCopy = fg_Move(pArenaCopy), WeakThis = fg_ThisActor(this).f_Weak(), nGatheredBytes](umint _iTransfer) mutable
			{
