...
namespace NMib
{
	namespace NWeb
	{
		...
		void CFastCGIConnectionActor::f_SetSocket(NPtr::TCSharedPointer<NNet::CSocket> const& _pSocket)
		{
			mp_Socket = fg_Move(*_pSocket);
			fp_ProcessState();
			mp_ProcessingThread = 0;
		}
		NConcurrency::TCContinuation<void> CFastCGIConnectionActor::f_Destroy()
		{
			mp_Socket.f_Close();
		...
		void CFastCGIConnectionActor::f_StateAdded(NNet::ENetTCPState _StateAdded)
		{
		...
		void CFastCGIConnectionActor::fp_SendData(NContainer::TCVector<uint8> const& _Data)
		{
		...
		void CFastCGIConnectionActor::f_SendStdOutput(NContainer::TCVector<uint8> const& _Data)
		{
			mp_RecordInfo.m_bSentStdOut = true;
			fp_SendStdOutput(_Data, ERequestType_StdOut);
		}
		
		void CFastCGIConnectionActor::f_SendStdError(NContainer::TCVector<uint8> const& _Data)
		{
		...
		{
			NPtr::TCSharedPointer<CFastCGIRequest> pRequest = fg_Construct(fg_ThisActor(this), *this);
		...
		void CFastCGIConnectionActor::fp_UpdateSend()
		{
			...
			if (ToSend)
			{
				...
				{
					// Discard old data
					NContainer::TCVector<uint8> OldData = fg_Move(mp_OutgoingData);
		...
		void CFastCGIConnectionActor::fp_ProcessState()
		{
			...
			if (StateAdded & NNet::ENetTCPState_Read && !mp_RecordInfo.m_bInvalid)
			{
				uint8 Data[4096];
				try
				{
				...
				while (true)
				{
					...
					if (Available < mp_IncomingData.f_GetLen() / 2)
					{
						...
						NContainer::TCVector<uint8> OldData = fg_Move(mp_IncomingData);
