...
namespace NMib
{
	...
	namespace NHTTP
	{
		...
		private:
			...
			NPtr::TCUniquePointer<NThread::CThreadObject> mp_pThread;
		...
		public:
			...
			// Thread safe
			void f_AddConnection( NPtr::TCUniquePointer<CConnection> _pConn );
		...
		private:
			...
			NPtr::TCUniquePointer<NThread::CThreadObject> mp_pListenThread;
	...
	//		NContainer::TCVector< TCUniquePointer<NThread::CThreadObject> > mp_lProcessingThreads;
		...
		aint CServer::CDetails::fp_Listen(NThread::CThreadObject* _pThread)
		{
			...
			try
			{
				NNet::CNetAddressTCPv4 TCPListenAddr;
			...
			NPtr::TCUniquePointer<NNet::CSocket> pIncomingSocket;
			...
			while(_pThread->f_GetState() != NThread::EThreadState_EventWantQuit)
			{
					while ( 1 )
					{
						...
						{
							NPtr::TCUniquePointer<CConnection> pNewConn = fg_Construct(fg_Move(pIncomingSocket));
		...
		// Thread safe
		void CConnectionWorker::f_AddConnection( NPtr::TCUniquePointer<CConnection> _pConn )
		{
			...
			while (_pThread->f_GetState() != NThread::EThreadState_EventWantQuit)
			{
					...
					;)
				{
					...
					else
					{
						...
						NPtr::TCUniquePointer<CConnection> pConn = NPtr::TCUniquePointer<CConnection>(&*CurIter);
