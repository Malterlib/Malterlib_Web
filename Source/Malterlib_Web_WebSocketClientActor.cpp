			...
			)
		{
			...
			mp_AddressResolver(&NNet::CResolveActor::f_Resolve, _ConnectToAddress, _PreferAddress)
				...
				(
					...
					, NConcurrency::TCAsyncResult<NNet::CNetAddress> &&_BindToResult
				)
				{
					if (!_Result)
					...
					else
					{
						...
						NPtr::TCSharedPointer<bool> pStateReceived = fg_Construct(false);
						...
						try
						{
							Pending.m_pSocket->f_AsyncConnect
								(
									fg_Move(Address)
									, [pStateReceived, pPending, WeakThis, Continuation, this, pRequest, _ConnectToAddress, _URI, _Origin, _Protocols](::NMib::NNet::ENetTCPState _StateAdded)
									{
											...
											(
												...
												, [WeakThis, pStateReceived, _StateAdded, this, pPending, Continuation, pRequest, _ConnectToAddress, _URI, _Origin, _Protocols]
												{
													...
													if (_StateAdded == NNet::ENetTCPState_Connected)
													{
														...
														NPtr::TCSharedPointer<NAtomic::TCAtomic<bool>> pRemovedPending = fg_Construct();
														ConnectionActor
															(
															...
															> This / [this, pRemovedPending, pPending](NConcurrency::TCAsyncResult<NConcurrency::CActorCallback> &&_Result)
															{
													...
													else if (_StateAdded == NNet::ENetTCPState_Closed)
													{
