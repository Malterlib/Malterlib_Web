...
namespace NMib
{
	namespace NWeb
	{
		...
		private:
			...
			NPtr::TCUniquePointer<CNGINXLauncher> mp_pNGINXLauncher;
			NPtr::TCUniquePointer<CFastCGIServer> mp_pFastCGIServer;
		...
		private:
			...
			public:
				...
				CConnection(NPtr::TCSharedPointer<CFastCGIRequest> const& _pRequest, CHTTPServerInternal& _Internal)
					...
					, mp_ContentStream(NContainer::TCVector<uint8>())
				...
				void f_HandleRequest(NPtr::TCSharedPointer<CConnection> const& _pThis, NContainer::TCMap<NStr::CStr, NStr::CStr> const& _Params)
				{
					...
					else if (fg_StrCmpNoCase(*pMethod, "POST") == 0)
					{
						...
						NPtr::TCSharedPointer<NContainer::TCVector<uint8>> pData = fg_Construct();
						
						mp_pRequest->f_OnStdInputRaw
							(
								[nPostBytes, pData, _pThis](uint8 const* _pData, mint _Len, bool _bEOF)
								{
									pData->f_Insert(_pData, _Len);
		...
		public:
			...
			bint f_Run(CHTTPServerOptions const& _Options)
			{
				...
				mp_pFastCGIServer
					= fg_Construct
					(
						[this](NPtr::TCSharedPointer<CFastCGIRequest> const& _pRequest)
						{
							...
							NPtr::TCSharedPointer<CConnection> pConnection = fg_Construct(_pRequest, *this);
	...
	//		Response += "\r\nConnection: close";
		...
		void CHTTPServerOptions::f_ParseCmdLine(NRegistry::CRegistry_CStr &_Params)
		{
			// Process command line args.
			NStr::CStr CurArg;		
			for (auto PIter = _Params.f_GetChildIterator(); PIter; ++PIter)
			{
				NRegistry::CRegistry_CStr const& CurChild = *PIter;
		...
		void CHTTPServerOptions::f_ParseSettings(NRegistry::CRegistry_CStr const& _Params)
		{
			fp_ParseSettings(_Params);
		}
		
		void CHTTPServerOptions::f_ParseSettings(NRegistry::CRegistryPreserve_CStr const& _Params)
		{
			fp_ParseSettings(_Params);
		}
		...
		CHTMLTemplate::CHTMLTemplate(NStr::CStr const& _Filename, bint _bExeFs)
		{
			...
			{
				NContainer::TCVector<uint8> lFileBytes;
		...
		CHTTPCachedConnection::CHTTPCachedConnection() : mp_ContentStream(NContainer::TCVector<uint8>())
		{
			f_Clear();
		}
