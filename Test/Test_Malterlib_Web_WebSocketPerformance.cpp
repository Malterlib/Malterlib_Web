...
namespace
{
	...
	struct CBenchState
	{
		...
		// Detach under lock and resolve outside it because inline resumption can replace the promise.
		...
		// Accept can deliver before the connection is assigned; wait before replying.
		...
		TCFuture<void> f_WaitClientReceived(uint64 _TargetBytes)
		{
...

			{
				...
				// Already-arrived data needs a ready promise; default futures cannot be awaited.
		...
		)
	{
		...
		auto ListenResult = _pState->m_ServerActor
			(
				...
				, ENetFlag_None
				// Bind to the handler actor; the remote server parks the ambient test thread without pumping it.
		...
		)
	{
		...
		// Retain the handler strongly in the callback; the weak binding alone would let a client-only handler die after setup.
	...
	struct TCBenchDriverActor : public CActor
	{
		...
		TCFuture<void> f_RunUpload(TCSharedPointerSupportWeak<CBenchState> _pState, TCSharedPointer<CIOByteVector const> _pChunk, uint64 _nBytes)
		{
			...
			// The last chunk is a view cut to the requested length, so the bytes sent are the bytes the measurement counts
			TCSharedPointer<NStream::CBinaryStorage const> pLastChunk;
			if (umint nLastBytes = umint(_nBytes % ChunkSize))
			{
				NStream::CBinaryStorage LastChunk;
				LastChunk.f_AppendShared(CSharedByteVector(_pChunk, 0, nLastBytes));
				pLastChunk = fg_Construct(fg_Move(LastChunk));
			}

			auto fSendChunk = [&](uint64 _iChunk) -> TCFuture<void>
				{
					if (_iChunk + 1 == nChunks && pLastChunk)
						return _pState->m_ClientSocket(&CWebSocketActor::f_SendBinaryStorage, fg_TempCopy(pLastChunk), 0);

					return _pState->m_ClientSocket(&CWebSocketActor::f_SendBinary, fg_TempCopy(_pChunk), 0);
				}
			;
			...
			{
				InFlight.f_InsertLast(fSendChunk(nQueued));
				++nQueued;
			}
			...
			for (uint64 iChunk = 0; iChunk < nChunks; ++iChunk)
			{
				...
				if (nQueued < nChunks)
				{
					InFlight[iSlot] = fSendChunk(nQueued);
					++nQueued;
				}
			}
	...
	class CWebSocketPerformance_Tests : public CTest
	{
		...
		{
			CStr RootDirectory = NFile::CFile::fs_GetProgramDirectory() / ("WebSocketPerf_{}"_f << _SuiteTag);
			...
			if (fs_BenchSchemeEnabled("wss_unix"))
			...
			if (fs_BenchSchemeEnabled("wss_masked"))
	...
	public:
		void f_DoTests()
		{
			...
			DMibTestSuite(CTestCategory("WebSocketRemoteServe") << CTestGroup("Manual"))
			{
				...
				if (!Host.f_IsEmpty())
				{
					...
					fg_SetupServer
						(
							RunLoopHelper
							, pState
							, CSocket::fs_ResolveAddress("{}:{}"_f << Host << Port, ENetAddressType_TCPv4)
							, ServerFactory
							, false
							, fg_ConstructActor<CBenchHandlerActorHighCpu>()
						)
					;
			...
			DMibTestSuite(CTestCategory("WebSocketRemoteThroughput") << CTestGroup("Manual"))
			{
